...
// the twitter REST API. (This is from the Documentation from github.com/dghubble/go-twitter/twitter)
...
// startingPlace The date that was the first day! (Day 264...)
const startingPlace = 264
...
var (
	...
	loc *time.Location
	// epoch The first ever day the bot was released. (Thursday, March 10, 2022 12:00:00AM ET)
	epoch = time.Unix(1646888400, 0)
)

func init() {
	f, err := os.OpenFile("wordleBot.log", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
...
// or an error (This is from the Documentation from github.com/dghubble/go-twitter/twitter)
...
func getWord(offset int) string {
	days := int(math.Round(time.Now().In(loc).Sub(epoch).Hours() / 24))
	return words[offset+days]
}
func getDay(offset int) string {
	return strconv.Itoa(offset + int(math.Round(time.Now().In(loc).Sub(epoch).Hours()/24)))
}
...
func timeerInit(client *twitter.Client) {
	t := time.Now()
	n := time.Date(t.Year(), t.Month(), t.Day(), 0, 0, 0, 0, loc)
	d := n.Sub(t)
	if d < 0 {
...
func midnightTask(client *twitter.Client) {
	tweet, _, err := client.Statuses.Update(fmt.Sprintf("The wordle of the day is: %s! #wordle #wordleoftheday #wordle%s", getWord(startingPlace), getDay(startingPlace)), nil)
	if err != nil {
		log.Fatalln(err)
	}
	log.Printf("Tweeted: %s\nAt: %s\n", tweet.Text, tweet.CreatedAt)
}
